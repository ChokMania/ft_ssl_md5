...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/10/26 15:18:35 by judumay           #+#    #+#             */
/*   Updated: 2020/10/29 10:49:28 by judumay          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_sha384.h"

static char	*ft_concat_sha384(t_var data)
{
	char		*str;
	char		*tmp;
	int			i;
	...
	i = 0;
	if (!(str = (char*)ft_memalloc(sizeof(char) * 113)))
		return (NULL);
	if (data.vb)
	{
...
static void	init_sha384_var(t_var *data, size_t tmp[80])
{
	int i;

	ft_bzero(data, sizeof(t_var));
	data->var_h[0] = 0xcbbb9d5dc1059ed8;
	data->var_h[1] = 0x629a292a367cd507;
	data->var_h[2] = 0x9159015a3070dd17;
	data->var_h[3] = 0x152fecd8f70e5939;
	data->var_h[4] = 0x67332667ffc00b31;
	data->var_h[5] = 0x8eb44a8768581511;
	data->var_h[6] = 0xdb0c2e0d64f98fa7;
	data->var_h[7] = 0x47b5481dbefa4fa4;
	i = -1;
	while (++i < 80)
		data->k[i] = (unsigned long)tmp[i];
}

static void	init_sha384(t_var *data)
{
	static size_t	tmp[80] = {0x428a2f98d728ae22, 0x7137449123ef65cd,
...
char		*ft_sha384(char *src, size_t size, int vb)
{
	...
	while (i < data.nb_block)
	{
		if (data.vb)
			ft_printf("\nBlock [ %u / %u ]\t str = %.128s\n", i + 1,
				data.nb_block, src + (i * 113));
		ft_loop_sha512(&tab[i * 32], &data);
		i++;
	}
	free(tab);
	return (ft_concat_sha384(data));
}
